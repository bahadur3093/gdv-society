...
import {
  DEFAULT_PER_SQFT_RATE,
  DEFAULT_SINKING_FUND_PERCENTAGE,
  SCREENS,
} from "@/utils";
...
export default function DashboardPage() {
  ...
  const sinkingFundPercentage =
    settings?.sinkingFundPercentage ?? DEFAULT_SINKING_FUND_PERCENTAGE;
  ...
  const handleUpdatePerSqFtRate = useCallback(async (rate: number) => {
    ...
    console.log(
      "[Dashboard] Per sq.ft rate update delegated to SocietyFinancialSettings component",
    );
  }, []);
  ...
  const handleUpdateSinkingFund = useCallback(async (percentage: number) => {
    // This is now handled by SocietyFinancialSettings component via Redux
    console.log(
      "[Dashboard] Sinking fund percentage update delegated to SocietyFinancialSettings component",
    );
  }, []);
  ...
  const handleUpdateTotalVillas = useCallback(async (total: number) => {
    // This is now handled by SocietyFinancialSettings component via Redux
    console.log(
      "[Dashboard] Total villas update delegated to SocietyFinancialSettings component",
    );
  }, []);
