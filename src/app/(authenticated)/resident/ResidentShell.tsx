...
import { ResidentRoutes } from "@/lib/routes/residents.route";

export default function ResidentShell({ children }: { children: ReactNode }) {
  const user = useUser();

  return (
    <main className="min-h-screen bg-slate-950 text-slate-100 transition-all duration-300 ease-in-out">
      <div className="min-h-screen flex flex-col lg:flex-row">
        <Sidebar user={user} navItems={ResidentRoutes} />
